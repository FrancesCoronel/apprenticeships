          ...
          {{#hits}}
            <a class="my-4 px-2 w-full md-w-1-2 lg-w-1-3" href="{{{link}}}" target="_blank" rel="noopener noreferrer">
              <div class="max-w-sm rounded overflow-hidden shadow-md hover-shadow-lg transition min-h-full">
                <img class="w-full" src="/images/apprenticeships/{{{image}}}" alt="{{{company}}} - Logo Image">
                <div class="px-6 py-4">
                ...
                <div class="px-6 py-4">
                  {{#location}}
                    <span class="inline-block bg-white-dark rounded-full px-3 py-1 text-sm text-grey-darker mr-2 mb-2">
                      {{ . }}
                    </span>
                  {{/location}}
                </div>
